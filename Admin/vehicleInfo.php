...
<body>
            ...
            <form method="post" action="./Admin/AddEditVehicleController.php">
                <div class="vehicle-admin form">
                    ...
                    <div style="grid-area: miles;">
                        ...
                        <input max="99999999.99" min="0.00" step="0.01" required type="number" id="miles" name="miles" value="<?= $Vehicle["PurchaseMileage"] ?>" onblur="validate('miles', 0, 99999999.99)">
                    </div>
                    <div style="grid-area: price;">
                        <label for="price">Purchase Price</label>
                        <input max="99999999.99" min="0.00" step="0.01" required type="number" id="price" name="price" value="<?= $Vehicle["PurchasePrice"] ?>" onblur="validate('price', 0, 99999999.99)">
                    </div>
                    ...
                    <div style="grid-area: userid">
                        ...
                        <select name="userid" id="userid">
                            <?php foreach ($Users as $UserList): ?>
                                <option <?= $Vehicle["UserID"] == $UserList["UserID"] ? "selected" : "" ?> value="<?= $UserList["UserID"] ?>"> <?= $UserList["FirstName"] . " " . $UserList["LastName"] ?> </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
            ...
            <form action="./Admin/AddEditFuelController.php" method="post">
                <div class="fuel form">
                    <div style="grid-area: source;">
                        ...
                        <input maxlength="50" required type="text" id="source" name="source" onblur="validate('source')">
                    </div>
                    <div style="grid-area: gallons;">
                        <label for="gallons">Gallons</label>
                        <input required min="0" max="99999999.99" type="number" step="0.01" id="gallons" name="gallons" onblur="validate('gallons', 0, 99999999.99)">
                    </div>
                    <div style="grid-area: cost;">
                        <label for="cost">Total Cost</label>
                        <input required min="0" max="99999999.99" type="number" step="0.01" id="cost" name="cost" onblur="validate('cost', 0, 99999999.99)">
                    </div>
                    <div style="grid-area: miles;">
                        <label for="miles">Miles</label>
                        <input required min="0" max="99999999.99" type="number" step="0.01" id="gasMiles" name="miles" onblur="validate('gasMiles', 0, 99999999.99)">
                    </div>
                    <div style="grid-area: date;">
                        <label for="purchaseDate">Purchase Date</label>
    ...
    <div class="modal" id="add-maint">
        <div class="modal-content">
            ...
            <form method="post" action="./Admin/AddEditMaintenanceController.php">
                <div class="maint form">
                    <div style="grid-area: vendor;">
                        <label for="vendor">Vendor</label>
                        <input maxlength="50" required type="text" id="vendor" name="vendor" onblur="validate('vendor')">
                    </div>
                    ...
                    <div style="grid-area: cost;">
                        <label for="maintCost">Total Cost</label>
                        <input required min="0" max="99999999.99" type="number" step="0.01" id="maintCost" name="cost" onblur="validate('maintCost', 0, 99999999.99)">
                    </div>
                    <div style="grid-area: milesBefore;">
                        <label for="milesBefore">Mileage Before</label>
                        <input required min="0" max="99999999.99" type="number" step="0.01" id="milesBefore" name="milesBefore", onblur="validate('milesBefore', 0, 99999999.99)">
                    </div>
    ...
    <!-- Sched MODAL VALID js -->
    <div class="modal" id="sched-maint">
        <div class="modal-content">
            <h2>Schedule Maintenance</h2>
            <form method="post" action="./Admin/AddEditMaintenanceController.php?schedule=1">
                <div class="sched form">
                    <div style="grid-area: vendor;">
                        <label for="schedVendor">Vendor</label>
                        <input maxlength="50" required type="text" id="schedVendor" name="vendor" onblur="validate('schedVendor')">
                    </div>
                    <div style="grid-area: vendorAddress;">
                        <label for="schedVendorAddress">Vendor Address</label>
                        <input maxlength="255" required type="text" id="schedVendorAddress" name="vendorAddress" onblur="validate('schedVendorAddress')">
                    </div>
                    <div style="grid-area: desc;">
                        <label for="schedDesc">Description</label>
                        <input maxlength="255" required type="text" id="schedDesc" name="desc" onblur="validate('schedDesc')">
                    </div>
                    <div style="grid-area: cost;">
                        <label for="schedCost">Total Cost</label>
                        <input required min="0" max="99999999.99" type="number" step="0.01" id="schedCost" name="cost" onblur="validate('schedCost', 0, 99999999.99)">
                    </div>
                    <div style="grid-area: dateStart;">
                        <label for="vendor">Date Scheduled</label>
                        <input required type="date" id="dateStart" name="dateStart">
                    </div>
                    <div style="grid-area: button;" class="buttons">
                        <input required type="hidden" name="vehicleID" value="<?= $Vehicle["VehicleID"] ?>">
                        <button type="submit">Add Scheduled Maintenance</button>
                        <button type="button" id="close-sched">Close</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="modal" id="delete-modal">
        <div class="modal-content">
            <div class="modal-message">
                <div class="header">
                    <h2>Delete <?= $Vehicle["Year"] . " " . $Vehicle["Brand"] . " " . $Vehicle["Model"] ?>?</h2>
                </div>
                <div class="prompt">
                ...
                <div class="buttons">
                    <form action="./Admin/DeleteController.php" method="get">
                        <input type="hidden" value="vehicle" name="action">
                        <input type="hidden" value="<?= $Vehicle["VehicleID"] ?>" name="id">
    ...
    <div class="body info p-10"> <!-- Table displaying the vehicle information -->
        <div class="info-head" style="grid-area: title;">
            <a href="./Admin/?action=vehicles" class="icon button">&#xE138</a>
                            ...
                            <?php if (count($fuel) > 0): ?>
                                <?php foreach ($fuel as $fuelItem): ?>
                                    <tr class="lined">
                                        ...
                                        <td class="text-center"><a href="./Admin/?action=gas&id=<?= $fuelItem["FuelID"] ?>">View</a></td>
                                    </tr>
                                <?php endforeach ?>
                            <?php else: ?>
                                <tr class="lined">
                            ...
                            <?php if (count($maintenance) > 0): ?>
                                <?php foreach ($maintenance as $maintItem): ?>
                                    <tr class="lined">
                                        ...
                                        <td>
                                            <p class="icon text-center"><?= $maintItem["Started"] == 0 ? "&#xE7B4" : (isset($maintItem["Completed"]) ? "&#xE182" : "&#xE80E") ?></p>
                                        </td>
                                        <td class="text-center"><a href="./Admin/?action=maint&id=<?= $maintItem["MaintenanceID"] ?>">View</a></td>
                                    </tr>
                                <?php endforeach ?>
                            <?php else: ?>
                                <tr class="lined">
